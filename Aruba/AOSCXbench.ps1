...
$CurrentLTSPatch = "10.07.1000"    # Define the LTS version you’re checking against

# Connect to the Aruba Switch
$Session = Connect-ArubaCX -Hostname $SwitchIP -Username $Username -Password $Password

if ($Session) {
    Write-Output "Connected to the Aruba CX switch at $SwitchIP."
    
    # Check firmware version against LTS version
    $Firmware = Get-ArubaCXFirmwareVersion -Session $Session
    ...
    Disconnect-ArubaCX -Session $Session
