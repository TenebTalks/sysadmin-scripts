...
#$Calendar = Read-Host -Prompt 'Input User who OWNs the calendar.'

#Edit this here for the list of users that are to RECEIVE permissions. Quotations are needed
#$User = Read-Host -Prompt 'Input User to RECEIVE permission' #"useremail1@xxx.com","useremail2@xxx.com", etc
...
ForEach ($Calendar in $CalendarList){
	...
	ForEach($User in $PermissionList){
		...
		Add-MailboxPermission -Identity $Calendar.Calendar -user $User.User -AccessRights $AccRights
	}
}